...
async function getDiscourseResource(
	context: any,
	actor: string,
	options: any,
	baseUrl: string,
...
async function getDiscourseEmailByUsername(
	context: any,
	actor: string,
	options: any,
	baseUrl: string,
 ...
 */
async function getDiscourseUserById(
	context: any,
	actor: string,
	options: any,
	baseUrl: any,
	...
	user.email =
		user.email ||
		(await getDiscourseEmailByUsername(
			context,
			actor,
			options,
			baseUrl,
 ...
 * @returns {(Object|Null)} Discourse user
 */
async function getDiscourseUserByUsername(
	context: any,
	actor: string,
	options: any,
	baseUrl: string,
 ...
 */
async function getDiscourseTopicById(
	context: any,
	actor: string,
	options: any,
	baseUrl: string,
 ...
 */
function getBaseUrl(event: any): any {
 ...
 */
function isEvent(object: any): boolean {
	return (
		object.type &&
 ...
 */
function isTopicEvent(event: any): boolean {
 ...
 */
function isPostEvent(event: any): boolean {
 ...
 */
function getTopicId(event: any): string {
	if (!isEvent(event)) {
		if (event.topic_id) {
 ...
 */
function getTopicMirrorId(baseUrl: string, event: any): string {
 ...
 */
function getMirrorId(baseUrl: string, event: any): string {
 ...
 */
function getThreadCard(mirrorId: string, data: any, options: any = {}): any {
	return {
		name: data.title,
		tags: [],
		links: {},
...
function normalizeMessage(message: string, baseUrl: string): string {
	/*
 ...
 */
function getEventCard(mirrorId: string, data: any, options: any): any {
...
function parseFullName(fullName: string): any {
...
async function getActor(
	...
	const remoteUser = userId
		? await getDiscourseUserById(context, actor, options, baseUrl, userId)
	...
	// have in the payload.
	const userDetails =
		(payload.topic && payload.topic.created_by) ||
...
async function getRemoteTopicData(
	context: any,
	actor: string,
	options: any,
	event: any,
...
async function getLocalElement(
	context: any,
	sequence: any,
	type: string,
	mirrorId: string,
): Promise<any> {
	const fromIndex = _.findLastIndex(sequence, (entry: any) => {
...
async function processTopicData(
	sequence: any,
	mirrorId: string,
	topic: any,
	options: any,
...
async function getTopicFromPostUrl(
	context: any,
	actor: string,
	options: any,
	url: string,
...
async function getPostFromPostUrl(
	context: any,
	actor: string,
	options: any,
	url: string,
...
export class DiscourseIntegration implements Integration {
	...
	public async mirror(
		...
		this.context.log.info('Mirroring Discourse', {
			url: discourseUrl,
			remote: card,
		});
	...
	async translate(
		...
		for (const post of posts) {
			...
			if (!eventCard) {
				...
				sequence.push(
					...utils.postEvent(sequence, postCard, newThreadCard, {
						actor: post.actorId,
					}),
				);
